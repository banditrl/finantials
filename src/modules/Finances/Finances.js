    ...
    updateDoc(selectedFinance.ref, {
      ...
      .then(() => {
        addDoc(collection(db, "history"), {
          ...
          window.alert("Value successfully spent");
          window.location.reload();
        });
      })
      .catch(() => {
        togglePopup();
        window.alert("Error spending value");
      });
  };

  const totalCard = () => {
    const totalValue = finances.reduce(
      (reducer, finance) => reducer + finance.value,
      0
    );
    const totalMaxValue = finances.reduce(
      (reducer, finance) => reducer + finance.maxValue,
      0
    );
    return (
      <div className="finance-card card-top-margin">
        ...
        <div className="finance-card-total">
          <div className="finance-max">{`Total remaining: R$ ${totalValue.toFixed(
            2
          )}`}</div>
          <div className="finance-max">{`Total configured: R$ ${totalMaxValue.toFixed(
            2
          )}`}</div>
        </div>
      </div>
      ...
      return (
        <div className="finance-card" key={`${index}${finance.name}.finance`}>
          ...
          <div className="finance-value">
            ...
            <div>{`Spent: R$ ${(finance.maxValue - finance.value).toFixed(
              2
            )}`}</div>
          </div>
          <div className="finance-card-footer">
            ...
            <div className="action-icons">
              <div className="icon" onClick={() => togglePopup(finance)}>
                <AiOutlineShopping />
              </div>
              <div className="icon" onClick={() => togglePopup(finance)}>
                <AiOutlineEdit />
